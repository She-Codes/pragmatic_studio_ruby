...
describe ProjectManager do
  ...
  it "adds money if there is an even number" do
    ...
    allow(ProjectAdjuster).to receive(:number_generator).and_return(2)
    allow_any_instance_of(ProjectAdjuster).to receive(:number_generator).and_return(2)
  ...
  it "deducts money if there is an odd number" do
    allow_any_instance_of(AmountGenerator).to receive(:generate).and_return(500)
    allow(ProjectAdjuster).to receive(:number_generator).and_return(1)
    allow_any_instance_of(ProjectAdjuster).to receive(:number_generator).and_return(1)
